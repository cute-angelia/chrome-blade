  ...
  var oPage = {
    ...
    functions: {
      fInitInfo: function () {
        ...
        // 磁力鸭
        if (oData.localhost.indexOf("cixiaoya") >= 0) {
          var myInterval = setInterval(function () {
            document.getElementById("link_input").className = "link_input"
            document.getElementById("link_input").type = "text"
            document.getElementById("link_input").style.width = "800px"
            clearInterval(myInterval)
          }, 1000);
        }

        // idope.se
        if (oData.localhost.indexOf("idope") >= 0) {
          var myInterval = setInterval(function () {
            let hashs2 = document.getElementsByClassName("hideinfohash")
            for (let i = 0; i < hashs2.length; i++) {
              if (i % 2 == 0) {
                const element = hashs2[i];

                let magnet = "magnet:?xt=urn:btih:" + element.textContent

                var biteUrl = document.createElement("a");
                biteUrl.text = element.nextElementSibling.textContent
                //biteUrl.setAttribute("class", "sendLxBitqiu ")
                biteUrl.setAttribute("data-href", magnet)
                biteUrl.setAttribute("href", "javascript:void(0);")
                //biteUrl.setAttribute("href", element.textContent)
                //element.nextElementSibling.nextElementSibling.appendChild(biteUrl)
                element.nextElementSibling.after(biteUrl)
              }
            }
        ...
        if (oData.localhost == "115.com") {
          var limtlz = setInterval(function () {
            var css = `
            .container-core .main-core{left:16px;}
            `;
            ...
            // remove();
            if (document.getElementById("js-main_mode")) {
              document.getElementById("js-main_mode").remove();
            }

          }, 1000);

          setTimeout(() => {
            clearInterval(limtlz);
          }, 8000);
        }
        ...
        if (oData.localhost == "www.tangbure.org") {
          setTimeout(function () {
            ...
            if (src) {
              ...
              $("#loadingCheck")
                .parent()
                .append('<a target="_blank" href="' + href + '">TUMBLR</a>');
            }
          }, 1000);
        }
        ...
        if (oData.localhost == "500px.com" || oData.localhost == "web.500px.com") {
          setInterval(function () {

            console.log($("img[class=photo-show__img]")[0].src)
        ...
        if (oData.localhost == "sexacg.com") {
          ...
          getp[getp.length] = document.getElementsByClassName(
            "su-quote-inner"
          )[0];
          for (var ai = 0; ai <= getp.length; ai++) {
            ...
            for (var ci = acgChildDriver.length - 1; ci >= 0; ci--) {
              ...
              if (bdtext && bdtext.split(/\s+/)[0].length == 8) {
                ...
                if (!bdtext.split(/\s+/)[0].match(/^[a-zA-Z0-9]{0,8}$/))
                  continue;
                a.setAttribute(
                  "href",
                  "https://pan.baidu.com/s/" + bdtext.split(/\s+/)[0]
                );
                ...
                bdnode = document.createTextNode(
                  "提取码：" + bdtext.split(/\s+/)[1] + "\n"
                );
              ...
              var takeacg = acgChildDriver[ci].textContent.match(
                /(\w{40})|(([A-Za-z0-9]{2,39})( ?)[\u4e00-\u9fa5 ]{2,}( ?)+(\w{2,37})\b)/g
              );
              if (takeacg) {
                for (var cj = 0; cj < takeacg.length; ++cj) {
                  //console.log(takeacg[cj]);
                  var achas = takeacg[cj]
                    .toString()
                    .replace(/(\s|[\u4e00-\u9fa5])+/g, "")
                    .trim();
                  if (achas.length >= 40) {
        ...
        if (
          oData.localhost == "www.hacg.me" ||
          oData.localhost == "www.hacg.fi" ||
          oData.localhost == "www.hacg.red" ||
          oData.localhost == "www.hacg.lol" ||
          oData.localhost == "www.hacg.li"
        ) {
          ...
          for (i = childDriver.length - 1; i >= 0; --i) {
            // O(n)
            if (
              (takeMe = childDriver[i].textContent.match(
                /([a-z0-9]{40})|(\w{32})|(([a-z0-9]{8,39})( ?)[\u4e00-\u9fa5 ]{2,}( ?)+([a-z0-9]{2,31})\b)/gi
              ))
            ) {
              for (j = 0; j < takeMe.length; ++j) {
                if (takeMe[j]) {
                  ...
                  var magnet =
                    takeMe[j].length >= 40 ? takeMe[j].trim() : takeMe[j];
                  A.setAttribute(
                    "href",
                    "magnet:?xt=urn:btih:" +
      ...
      fAddBox: function () {
        ...
        oD_button.onclick = function () {
          ...
          if (oD_hash === "") {
            ...
            oD_link.textContent = "\n磁力链接";
            oD_link2.href =
              "http://www.torrent.org.cn/Home/torrent/download.html?hash=" +
              oD_hash;
            oD_link2.textContent = "下载种子";
